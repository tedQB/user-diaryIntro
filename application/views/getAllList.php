	...
	<ul class="mui-table-view" id="diary">	
		<?php if($args['getAllList']!='null'){
			foreach ($args['getAllList'] as $key => $value) { ?>
				<li class="mui-table-view-cell" style="<?php if($value['isRec']){ echo 'display:none'; }?>">
					<p class="clipped">  
						<?=$value['name'] ?>
						<?=$value['hasTime'] ?>
					...
					<dl class="cont <?php if($value['isRec']){ echo 'none'; }else{ echo ''; }?> ">
						<dd><?=$value['cont1'] ?></dd>
						<dd class="break">----------------------</dd>
						<dd><?=$value['cont2'] ?></dd>
						<dd class="break">----------------------</dd>
						...
						<dd><?=$value['cont7'] ?></dd>
						<dd class="break">----------------------</dd>
						<dd>日期:<?=$value['subDate'] ?> 日报成绩<?=$value['sum']?>分。<!--<?=$value['tip']?>-->日报成绩为<?=$value['sum']?>分。<!--<?=$value['tip']?>--></dd>
						<dd class="break">----------------------</dd>
						<?php 
...
<script id="list-li" type="text/x-handlebars-template">
	{{#each arr}}
	<li class="mui-table-view-cell"	style="{{#if isRec}}display:none{{/if}}">
		<p class="clipped">
			{{name}}
			{{hasTime}} <button type="button" class="recommenbtn btn-default" data-id="{{id}}">no</button>
		</p>
		<dl class="cont">
			<dd>{{cont1}}</dd>
			<dd class="break">----------------------</dd>
...
<script>
  seajs.use(['http://style.aliunicorn.com/mobile/ae/common/panel/1.0.1/pandel.js', 'http://style.aliunicorn.com/js/6v/lib/gallery/jquery/jquery.js','http://style.aliunicorn.com/js/6v/lib/gallery/handlebars/handlebars.js'], function(panel, $ ,Handlebars) {
